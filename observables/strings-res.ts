...
const STRINGS_ZH_CN = {
    ...
    CALC_CK_ERROR_MSGS: {
        ...
        INSUFFICENT_OPERANDS: STRINGS_ZH_CN_ERROR_PREFIXES.CALC + "操作数数量不足",
        OPERAND_STACK: STRINGS_ZH_CN_ERROR_PREFIXES.CALC + "操作数栈终止长度不为1",
    }
}
...
const STRINGS_EN = {
    ...
    PARSE_ERROR_MSGS: {
        MISSING_L_BRACKET: STRINGS_EN_ERROR_PREFIXES.PARSE + "Missing L bracket (",
        MISSING_R_BRACKET: STRINGS_EN_ERROR_PREFIXES.PARSE + "Missing R bracket )",
        UNEXPECTED_ENTRY: STRINGS_EN_ERROR_PREFIXES.PARSE + "Invalid key entry",
        INSUFFICENT_OPERANDS: STRINGS_EN_ERROR_PREFIXES.PARSE + "Insufficient operand(s)",
        MISSING_COMMA: STRINGS_EN_ERROR_PREFIXES.PARSE + "Missing comma ,",
        TOO_MANY_DEGREE_SYMBOL: STRINGS_EN_ERROR_PREFIXES.PARSE + "Too many degree symbols °",
        TOO_MANY_DECIMAL_POINT: STRINGS_EN_ERROR_PREFIXES.PARSE + "Too many decimal points .",
        EMPTY_BRACKETS: STRINGS_EN_ERROR_PREFIXES.PARSE + "Empty brackets",
    },
    CALC_CK_ERROR_MSGS: {
        GOT_NOT_POSITIVE: STRINGS_EN_ERROR_PREFIXES.CALC + "Got zero or negative",
        SQRT: STRINGS_EN_ERROR_PREFIXES.CALC + "sqrt() got negative",
        TAN: STRINGS_EN_ERROR_PREFIXES.CALC + "tan() got odd multiples of π/2",
        ASINACOS: STRINGS_EN_ERROR_PREFIXES.CALC + "arcsin() or arccos() got operand out of [-1,1]",
        FACT: STRINGS_EN_ERROR_PREFIXES.CALC + "factorial did not get non-negative integer",
        INV: STRINGS_EN_ERROR_PREFIXES.CALC + "Cannot invert 0",
        COMBINE_NOT_NNINT: STRINGS_EN_ERROR_PREFIXES.CALC + "nPr or nCr did not get non-negative integers",
        COMBINE_X_LT_Y: STRINGS_EN_ERROR_PREFIXES.CALC + "nPr or nCr got n<r",
        POW: STRINGS_EN_ERROR_PREFIXES.CALC + "Cannot calculate non-integer power of a negative number",
        ROOT_X_ZERO: STRINGS_EN_ERROR_PREFIXES.CALC + "Cannot calculate 0 root",
        ROOT_Y_NEG_X_NOT_ODD_INT: STRINGS_EN_ERROR_PREFIXES.CALC + "Cannot calculate non-odd root of a negative number",
        DIV: STRINGS_EN_ERROR_PREFIXES.CALC + "Cannot divide by zero",
        REC: STRINGS_EN_ERROR_PREFIXES.CALC + "First arg of Rec() cannot be negative",

        CREATE_DEGREE: STRINGS_EN_ERROR_PREFIXES.CALC + "Minutes and seconds cannot be negative",
        CREATE_FRAC: STRINGS_EN_ERROR_PREFIXES.CALC + "Denominator cannot be zero",

        INSUFFICENT_OPERANDS: STRINGS_EN_ERROR_PREFIXES.CALC + "Insufficient operands",
        OPERAND_STACK: STRINGS_EN_ERROR_PREFIXES.CALC + "Operand stack remaining count is not 1",
    }
}
